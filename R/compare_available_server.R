...
compare_available_server <- function(required_packages_df, server_packages_df) {
  ...
  required_packages_df %>%
    ...
    mutate(min_package_version
           = str_extract(package_version_required, "\\(([^)]+)\\)")) %>%
    # get brackets version
    mutate(min_package_version =
             str_extract(min_package_version, "[0-9\\.]+")) %>%
