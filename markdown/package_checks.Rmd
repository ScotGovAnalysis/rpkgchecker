...
main_package_version <- package_requirements %>% filter(package==search_package) %>% pull(latest_version)
```


## Package Requirements for [`r paste0(search_package, " v", main_package_version)`](`r search_package_url`)
...
if (nrow(need_upgrading_on_server) > 0) {
  ...
  for (i in 1:nrow(need_upgrading_on_server)) {
    ...
    need_higher_version <- all_package_df %>%
      ...
      mutate(combined_p_v = paste0(package, "(", version, ")")) %>%
      pull(combined_p_v)
    need_higher_version <- capture.output(cat(need_higher_version, sep = ", "))
    cat(paste("<b>", the_package, "</b>", "currently on server", "<b>", the_server_version, "</b>- requires upgrading", "to latest version <b>", need_upgrading_on_server[[i, "latest_version"]], "</b>", need_upgrading_on_server[[i, "package_url"]], "<br>(upgrade required by", need_higher_version, ")<br>"))
  }
} else {
